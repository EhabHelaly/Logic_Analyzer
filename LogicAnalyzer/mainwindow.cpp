   ...
   ui(new Ui::MainWindow)
{
   ...
   connect(serial_timer, &QTimer::timeout, [this]{
      this->checkHardware();
   });
   serial_timer->start();
   ...
   connect(plotting_timer, &QTimer::timeout, [this]{
      this->plotRun();
   });
}
   ...
   if (serialReceive(buffer + buffer_offset, size) )
   {
      switch (mode)
      {
         ...
         case LA_MODE_ANALOG:
         {
            ...
            {
               adc_raw_value  = 0x3FF & *( (uint16_t *)(buffer + i) ); /* Get 16 bit value (low and high bytes of
                                                                          ADC)(Little Endian) */
...
void MainWindow::plotAnalog(double adc_disp_value, double t_last, double dt, uint32_t i)
{
   ...
   else     /* following samples */
   {
      ...
      if (  (  IS_SIGNAL_HIGH(value)
            && (adc_disp_value < (adc_scale * ui->spin_schmittLow->value() ) ) )
         || (  IS_SIGNAL_LOW(value)
            && (adc_disp_value > (adc_scale * ui->spin_schmittHigh->value() ) ) ) )
      {
...
void MainWindow::showDecodedSignalInfo(uint8_t signal_index)
{
   ...
   switch (decode_pops[signal_index]->currentIndex() )
   {
      case LA_DECODE_I2C:
      {
         ...
         logs[signal_index]->append("=======");
         for (QVector<I2cFrame>::iterator frame = signals_d[signal_index].i2cMessages.begin();
              frame < signals_d[signal_index].i2cMessages.end();
              frame++)
         {
            logs[signal_index]->append(QString("%1: %2 ms , 0x%3 (%4) (%5)")
                                          .arg(++frame_index)
                                          .arg(frame->time)
                                          .arg(QString("%1").arg(frame->address.data >> 1, 2, 16, QChar(
                                                                    '0') ).toUpper() )
            ...
            for (uint32_t index = 0; index < frame->dataBytes.size(); index++)
            {
               logs[signal_index]->append(QString("\t%1: 0x%2 (%3)")
                                             .arg(index + 1)
                                             .arg(QString("%1").arg(frame->dataBytes[index].data, 2, 16,
                                                                    QChar('0') ).toUpper() )
      ...
      case LA_DECODE_SPI:
      {
         ...
         logs[signal_index]->append("=======");
         for (QVector<SpiFrame>::iterator frame = signals_d[signal_index].spiMessages.begin();
              frame < signals_d[signal_index].spiMessages.end();
              frame++)
         {
      ...
      case LA_DECODE_NEC:
      {
         ...
         logs[signal_index]->append("=======");
         for (QVector<NecFrame>::iterator frame = signals_d[signal_index].necMessages.begin();
              frame < signals_d[signal_index].necMessages.end();
              frame++)
         {
...
void MainWindow::showPulseInfo(double x, double y)
{
   ...
   if (  (found != (signals_d[b].data->end() - 1) )
      && ( (found)->key <= x) )                                    /* found time stamp */
   {
...
void MainWindow::showAmplitudeInfo(double x)
{
   ...
   if (  (found != (signals_a[0].data->end() - 1) )
      && ( (found)->key <= x) )                                    /* found time stamp */
   {
      ui->tabWidget_2->setCurrentIndex(0);
      ui->log->append("=======");
      ...
      if (  (found != (signals_a[0].binary.data->end() - 1) )
         && ( (found)->key <= x) )                                            /* found time stamp */
      {
         double pulseWidth = (found + 1)->key - (found)->key;
         ui->log->append(QString("Pulse Start\t: %1 ms").arg( (found)->key) );
...
void MainWindow::on_plot_mousePress(QMouseEvent *event)
{
   if (event->button() == Qt::RightButton)
   {
      ...
      {
         if (  (mode == LA_MODE_ANALOG)
            || (mode == LA_MODE_ANALOG_X2) )
         {
            if (signals_a.size() )
            {
               showAmplitudeInfo(x);
...
void MainWindow::on_pushB_connect_pressed()
{
   ...
   else
   {
      ...
      if (serialOpen(portNumber, getBaudrate() ) )
      {
         ...
         if (  serialReceive(temp_buffer, size)
            && (size == 1)
            && (temp_buffer[0] == LA_CMD_SYNC_CHECK) )
         {
         ...
         else       /* no response received or invalid response received */
         {
            ...
            ui->tabWidget_2->setCurrentIndex(0);
            ui->log->append(
               "<span style='color: red'>Device is not responding, check hardware connections and Clock Frequency settings</span>");
         }
      }
      else
      {
...
void MainWindow::on_pushB_capture_pressed()
{
   ...
   else
   {
      ...
      ui->tabWidget_2->setTabEnabled(1, false);                 /* disable settings tab, changes current tab if tab is
                                                                   disabled */
      ...
      switch (mode)
      {
         ...
         default:
         {
            ...
            if (ui->checkB_schmitt->isChecked() )
            {
               ...
               ui->plot->graph(LA_GRAPH_ANALOG_SCHMITTLOW)->addData(ui->spin_msec->maximum(),
                                                                    adc_scale * ui->spin_schmittLow->value() );
               /* Schmitt Trigger comparator High */
               ui->plot->graph(LA_GRAPH_ANALOG_SCHMITTHIGH)->addData(0, adc_scale * ui->spin_schmittHigh->value() );
               ui->plot->graph(LA_GRAPH_ANALOG_SCHMITTHIGH)->addData(ui->spin_msec->maximum(),
                                                                     adc_scale * ui->spin_schmittHigh->value() );
            }
            /* show y-axis data labels by setting its color to white */
            ui->plot->yAxis->setTickLabelColor(color_plot[1 - theme_settings]);
            /* Adjust Y axis to suit current mode */
            ui->plot->yAxis->setRange(0, adc_scale > 255.0 ? 1200.0 : adc_scale == 255.0 ? 290.0 : 8.0); /* max data +
                                                                                                            17% */
