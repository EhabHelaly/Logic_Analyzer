...
#define IS_SIGNAL_HIGH(s) ( (s) == ( (int)(s) ) )       /* Logic High is represented by a whole number Ex: 1.0 2.0 3.0 ... */
#define IS_SIGNAL_LOW(s)  ( (s) != ( (int)(s) ) )       /* Logic Low is represented by a decimal number Ex: 0.5 1.5 2.5 ... */

/* Boolean expression that compares signal width to reference width with tolerance from 0.0 to 1.0 */
#define CMP_SIGNAL(signal,reference,tolerance) ((((reference)*(1-(tolerance)))<=(signal))&&((signal)<=((reference)*(1+(tolerance)))))
...
struct I2cFrame{
    double time = 0;
    I2cField address;        /* data -> 7 bit address + R/W bit , ack -> denotes whether slave replied with acknowledgment or no */
