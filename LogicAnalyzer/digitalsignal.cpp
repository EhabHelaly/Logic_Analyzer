...
void DigitalSignal::decodeI2C(double tStart, double tEnd)
{
    ...
    while( a < aEnd  )
    {
       ...
       if(IS_SIGNAL_HIGH(i2c_start->value)) /* a valid start condition is found ? */
       {
           ...
           }while(  /* loop until SDA line changes while SCL is High or until signal ends */
           ...
           if(IS_SIGNAL_HIGH(i2c_repeatedStart_or_stop->value)) /* a valid  repeated start or stop condition is found ? */
           {
               uint32_t i2c_bits = i2c_repeatedStart_or_stop-i2c_start;
               i2c_bits = i2c_bits/WAVE_NUM_POINTS - 1;
               if( (i2c_bits>=I2C_FIELD_BITS) && (i2c_bits%I2C_FIELD_BITS == 0) )   /* a valid number of clk cycles between start and stop ? */
               {
                   I2cFrame frame;
                   ...
                   for(uint8_t index =1; index<i2c_bits/I2C_FIELD_BITS; index++) /* loops up to number of i2c fields - 1 */
                   {
                       i2c_field = 0;
                       for(int index =0; index<I2C_FIELD_BITS; index++) /* loops 9 times */
                       {
...
void DigitalSignal::decodeNEC(double tStart, double tEnd)
{
    ...
    uint32_t message=0;
    necMessages.clear();
    if(!_getSampleIndices(data, tStart, tEnd, aStart, aEnd) || (waves<NEC_FRAME_PULSES) )
    {
        return;
    }
    ...
    for(int a=aStart; a<aEnd; a+=WAVE_NUM_POINTS)
    {
        if( ( a<=(aEnd-(NEC_FRAME_PULSES*WAVE_NUM_POINTS)) )   /* a valid number of waves is left? */
            &&( CMP_SIGNAL(_getWidth(a), NEC_START_LOW, NEC_TOLERANCE) || CMP_SIGNAL(_getWidth(a), NECX2_START_LOW, NEC_TOLERANCE) )
            &&CMP_SIGNAL(_getWidth(a+PULSE_NUM_POINTS), NEC_START_HIGH, NEC_TOLERANCE) )
        {
            ...
            if(!errorFlag)
            {
                ...
                if ( (frame.data^data_inverse) == 0xFF)    /* check if frame data is correct */
                {
                  ...
                  && CMP_SIGNAL(TIME(a)-necMessages.back().time, NEC_INTERFRAME_SPACE*(necMessages.back().repeat+1), NEC_TOLERANCE)
        )
        {
