...
void AnalogSignal::analyze(double tStart, double tEnd)
{
   ...
   std::pair<QCPGraphData *,
             QCPGraphData *> data_min_max =
      std::minmax_element(data->begin(), data->end(), [](QCPGraphData const &s1, QCPGraphData const &s2) {
   ...
   /* avg amp or offset */
   offset = std::accumulate(data->begin(), data->end(), 0.0, [](double sum, QCPGraphData const &s) {
      return(sum + s.value);
   }) / (double)data->size();
   /* max rate of change */
   delta_max = *std::max_element(data->begin(), data->end() - 1, [](QCPGraphData const &s1, QCPGraphData const &s2) {
      return(std::abs( ( (&s1) + 1)->value - s1.value) < std::abs( ( (&s2) + 1)->value - s2.value) );
   });
