...
static u8          boost_data    = 0;           /* contains combined samples used in DIGITAL_X2 modes */
static volatile u8 boost_counter = 0;           /* controls when to transmit combined samples in DIGITAL_X2 modes */
   ...
   while (1)
   {
      ...
      switch (mode)
      {
         case LA_CMD_ANALOG:
         {
            ...
            while (ALL_CLR(LA_REG_UCSRA, BM(LA_BIT_UDRE) ) );  /* Blocks if previous transmission is not finished */
         ...
         {
            boost_data = (boost_data << 4) | (LA_REG_PIN & 0x0F); /* combine 2 samples (4 bits each) in 1 byte */
